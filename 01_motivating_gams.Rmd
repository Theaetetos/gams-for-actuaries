...
$$
In English, some _response variable_ $y$ is modeled as the sum of an _intercept_ term $\beta_0$ and $n$ _predictor variables_ $x_1,...,x_n$ with random _error_ $\epsilon$ distributed normally with mean 0 and variance $\sigma^2$.
...
This residual plot makes the cause of the grouping issue very clear: the price slope for diamonds of clarity I1 looks to be quite different from the slopes of the rest of the clarity ratings.
...
```{r}
# CJ stands for 'Cross Join', and creates a data.table that is the cartesian product of all of
# the input vectors
pdat <- CJ(
...
Here, I am using the `ti()` (short for <u>t</u>ensor <u>i</u>nteraction) constructor because this is the recommended way to create smooth interaction terms in the context of GAMs.
...
```{r}
# if you want to be able to use your computer during model fitting, take away a core or two
