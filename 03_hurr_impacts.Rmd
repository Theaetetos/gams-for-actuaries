...
There's a sneaky pattern in the residuals: Winter Storm Uri, the unofficial name for the ice storms that hit Texas in 2021.
...
ggplot(dat) +
  ...
  labs(title = 'Initial Model Residuals Near Landfall') +
  mytheme +
...
There are two things we need to test for and correct before we can make valid statistical inferences from the model, however.
...
A valid model will have (almost) all of the vertical lines (very close to) within the confidence bands.
...
hdat <- as.data.table(
  ...
  `:=`(
    ...
    sig = .upper_ci < 0 | .lower_ci > 0
  )
]
ggplot(hdat, mapping = aes(ind)) +
  geom_hline(yintercept = 1, linetype = 'dashed') +
  geom_ribbon(
    aes(ymin = exp(.lower_ci), ymax = exp(.upper_ci)),
    alpha = .2
  ) +
  geom_line(
    aes(y = exp(.estimate), group = 1, color = sig),
    linewidth = 2,
    show.legend = F
  ) +
...
fctrs <- as.data.table(
  ...
  ,
  .(ds = seq(from = landfall - 1, length.out = 22, by = 1), .estimate,
    .lower_ci, .upper_ci)
][
  !(ds < landfall),
][
...
Not quite twelve basis points may seem unsatisfying after all that work, but professionals have to track impacts down below ten basis points, and I've been on projects that were more work, less statistically sound, and had less impact than we see here.
That's the life of a forecasting actuary...if you like it, you love it; if not, maybe look into another field!
